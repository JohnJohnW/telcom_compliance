...
function ScrollMesh() {
  ...
  const scrollRef = useRef<Mesh>(null)
  ...
  useFrame((state) => {
    ...
    if (scrollRef.current) {
      scrollRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 0.8) * 0.05
    }
  })
  ...
  return (
    <group ref={groupRef}>
      {/* Scroll body */}
      <mesh ref={scrollRef} position={[0, 0, 0]}>
      ...
      <mesh position={[0, 0.6, 0]}>
        <cylinderGeometry args={[0.45, 0.4, 0.1, 8]} />
        <meshStandardMaterial
          color="#7c3aed"
          emissive="#5b21b6"
...
export function Scroll3D({ className = '' }: Scroll3DProps) {
